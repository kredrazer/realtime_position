...
dwTimestamp_t dwSetDelay(dwDeviceTypes_t *dev, const dwTimestamp_t *delay)
{
    ...
    {
        // in idle, ignore
        dwTimestamp_t zero = {.timeFull = 0};
        return zero;
    }
    uint8_t delayBytes[5];
    dwTimestamp_t futureTime;
    dwGetSystemTimestamp(dev, &futureTime);
    futureTime.timeFull += delay->timeFull;
    memcpy(delayBytes, futureTime.timeRaw, sizeof(futureTime.timeRaw));
    delayBytes[0] = 0;
    delayBytes[1] &= 0xFE;
    dwSpiWrite(dev, DX_TIME, NO_SUB, delayBytes, LEN_DX_TIME);
    // adjust expected time with configured antenna delay
    memcpy(futureTime.timeRaw, delayBytes, sizeof(futureTime.timeRaw));
...
void dwAttachErrorHandler(dwDeviceTypes_t *dev, dwHandler_t handler)
{
...
void dwAttachReceiveTimestampAvailable(dwDeviceTypes_t *dev, dwHandler_t handler)
{
...
{
    dev->antennaDelay.timeFull = delay.timeFull;
}

char *dwStrError(int error)
{
    if (error == DW_ERROR_OK)
        return "No error";
    else if (error == DW_ERROR_WRONG_ID)
